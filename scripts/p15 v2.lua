...
local foodVendor = {name = "Innkeeper Hearthstove", x = -5376, y = -2973, z = 324}

local vendor = {name = "Morhan Coppertongue", x = -5350, y = -2957, z = 324}
        ...
        ticker = C_Timer.NewTicker(0.3, function ()
           ...
           if inPoint then
            ...
            if GossipFrame:IsShown() then
                C_GossipInfo.SelectOption(96526)
            end
            if MerchantFrame:IsShown() then
                local itemsQuantity = GetMerchantNumItems()
                for i = 1, itemsQuantity do
                    local name = select(1, GetMerchantItemInfo(i))
                    if GetItemCount(select(1, GetItemInfo(Food.water))) >= 20
                        and 
                       GetItemCount(select(1, GetItemInfo(Food.food))) >= 20
                    then
                        target = nil
                        CancelActiveTicker()
                        Start()
                    ...
                    if name == Food.food then
                        BuyMerchantItem(i, 5)
                    elseif name == Food.water then
                        BuyMerchantItem(i, 5)
                    end
                end
            end
...
function setTargets()
    ...
    if #objects > 0 then
        for _, v in ipairs(objects) do
            if nn.UnitName(v) == "Forest Lurker" or nn.UnitName(v) == "Mountain Boar" or nn.UnitName(v) == "Elder Black Bear" then -- Фильтруем таблицу по нужным нам юнитам.
