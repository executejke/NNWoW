...
local foodVendor = {name = "Innkeeper Helbrek", x = -3825, y = -830, z = 11}

local vendor = {name = "Edwina Monzor", x = -3752, y = -844, z = 11}
...
-- function MoveWithRawPath(path)
--     CancelActiveTicker()
--     local i = 1
--     ticker = C_Timer.NewTicker(0.1, function ()
--         local px, py, pz = ObjectPosition("player")
        
--         ClickToMove(path[i].x, path[i].y, path[i].z)
--         if math.abs(math.ceil(px) - path[i].x) <= 1 and math.abs(math.ceil(py) - path[i].y) <= 1 and math.abs(math.ceil(pz) - path[i].z) <= 1 then
--             i = i + 1
--         end
--     end)

--     local toVendor = function ()
--         CancelActiveTicker()
--         local i = 1
--         ticker = C_Timer.NewTicker(0.1, function ()
--             local
--         end)
--     end
--     -- TOVENDOR OUTVENDOR дВЕ ФУНКЦИИ СОЗДАТЬ наверно
-- end
...
function setTargets()
    ...
    if #objects > 0 then
        for _, v in ipairs(objects) do
            if nn.UnitName(v) == "Red Whelp"
            or nn.UnitName(v) == "Lost Whelp"
            or nn.UnitName(v) == "Crimson Whelp"
            or nn.UnitName(v) == "Black Ooze"
            or nn.UnitName(v) == "Monstrous Ooze"
            or nn.UnitName(v) == "Highland Raptor"
            or nn.UnitName(v) == "Highland Lashtail"
            or nn.UnitName(v) == "Highland Scytheclaw" then -- Фильтруем таблицу по нужным нам юнитам.
